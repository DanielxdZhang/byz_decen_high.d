...
get_parameter <- function(Lambda, Gamma){
  ...
  for(l1 in 1:length(Lambda)){
    ...
    for(l2 in 1:length(Gamma)){
      ...
      ###initial gradients
      Y_tilde_N <- dep_N <- Indi_N <- M_dif_N <- rep(0, N)
      for(i in 1:M){
        dep_N[((i-1)*n+1):(i*n)] <- Y[((i-1)*n+1):(i*n)] - X[((i-1)*n+1):(i*n), ] %*% theta_t[((i-1)*p+1):(i*p)]
        Indi_N[((i-1)*n+1):(i*n)] <- (abs(dep_N[((i-1)*n+1):(i*n)]) <= htau)
        M_dif_N[((i-1)*n+1):(i*n)] <- htau*sign(dep_N[((i-1)*n+1):(i*n)])*(abs(dep_N[((i-1)*n+1):(i*n)]) > htau) + 
          dep_N[((i-1)*n+1):(i*n)]*Indi_N[((i-1)*n+1):(i*n)]
        Y_tilde_N[((i-1)*n+1):(i*n)] <- X[((i-1)*n+1):(i*n), ] %*% theta_t[((i-1)*p+1):(i*p)] + 
          M_dif_N[((i-1)*n+1):(i*n)] / mean(Indi_N[((i-1)*n+1):(i*n)])
      }
      for(i in 1:M){
...
for(r in 1:R){
  ...
  Y_tilde_N <- dep_N <- Indi_N <- M_dif_N <- rep(0, N)
  Y_tilde_M <- dep_M <- Indi_M <- M_dif_M <- rep(0, N)
  Y_tilde_T <- dep_T <- Indi_T <- M_dif_T <- rep(0, N)
  Y_tilde_K <- dep_K <- Indi_K <- M_dif_K <- rep(0, N)
  ...
  for(i in 1:M){
    ...
    Indi_N[((i-1)*n+1):(i*n)] <- (abs(dep_N[((i-1)*n+1):(i*n)]) <= htau)
    Indi_M[((i-1)*n+1):(i*n)] <- (abs(dep_M[((i-1)*n+1):(i*n)]) <= htau)
    Indi_T[((i-1)*n+1):(i*n)] <- (abs(dep_T[((i-1)*n+1):(i*n)]) <= htau)
    Indi_K[((i-1)*n+1):(i*n)] <- (abs(dep_K[((i-1)*n+1):(i*n)]) <= htau)
    
    M_dif_N[((i-1)*n+1):(i*n)] <- htau*sign(dep_N[((i-1)*n+1):(i*n)])*(abs(dep_N[((i-1)*n+1):(i*n)]) > htau) + 
      dep_N[((i-1)*n+1):(i*n)]*Indi_N[((i-1)*n+1):(i*n)]
    M_dif_M[((i-1)*n+1):(i*n)] <- htau*sign(dep_M[((i-1)*n+1):(i*n)])*(abs(dep_M[((i-1)*n+1):(i*n)]) > htau) + 
      dep_M[((i-1)*n+1):(i*n)]*Indi_M[((i-1)*n+1):(i*n)]
    M_dif_T[((i-1)*n+1):(i*n)] <- htau*sign(dep_T[((i-1)*n+1):(i*n)])*(abs(dep_T[((i-1)*n+1):(i*n)]) > htau) + 
      dep_T[((i-1)*n+1):(i*n)]*Indi_T[((i-1)*n+1):(i*n)]
    M_dif_K[((i-1)*n+1):(i*n)] <- htau*sign(dep_K[((i-1)*n+1):(i*n)])*(abs(dep_K[((i-1)*n+1):(i*n)]) > htau) + 
      dep_K[((i-1)*n+1):(i*n)]*Indi_K[((i-1)*n+1):(i*n)]
    
    Y_tilde_N[((i-1)*n+1):(i*n)] <- X[((i-1)*n+1):(i*n), ] %*% theta_t[((i-1)*p+1):(i*p), r] + 
      M_dif_N[((i-1)*n+1):(i*n)] / mean(Indi_N[((i-1)*n+1):(i*n)])
    Y_tilde_M[((i-1)*n+1):(i*n)] <- X[((i-1)*n+1):(i*n), ] %*% theta_t_M[((i-1)*p+1):(i*p), r] + 
      M_dif_M[((i-1)*n+1):(i*n)] / mean(Indi_M[((i-1)*n+1):(i*n)])
    Y_tilde_T[((i-1)*n+1):(i*n)] <- X[((i-1)*n+1):(i*n), ] %*% theta_t_T[((i-1)*p+1):(i*p), r] + 
      M_dif_T[((i-1)*n+1):(i*n)] / mean(Indi_T[((i-1)*n+1):(i*n)])
    Y_tilde_K[((i-1)*n+1):(i*n)] <- X[((i-1)*n+1):(i*n), ] %*% theta_t_K[((i-1)*p+1):(i*p), r] + 
      M_dif_K[((i-1)*n+1):(i*n)] / mean(Indi_K[((i-1)*n+1):(i*n)])
  }
  ...
  for(t in 1:tau){
    ...
    dep <- Y - X %*% theta_glo[, r]
    Indi <- (abs(dep) <= htau)
    M_dif <- htau*sign(dep)*(abs(dep) > htau) + dep*Indi
    Y_tilde <- X %*% theta_glo[, r] + M_dif / mean(Indi)
